...
nmapv4_report = NmapParser.parse_fromfile('/tmp/v4-6test/ip6-tcp-10.xml')
v6xml = open("/tmp/v4-6test/hostnames-ip6-tcp-10.xml","r")
...
for eachv4host in nmapv4_report.hosts:
    ...
    for eachv6host in v6results['nmaprun']['host']:
        ...
        if hostv4name == hostv6name:
            ...
            continue
    if match != True:
        print("Hmmm.... hostnames don't match for this one... ????")
    ...
    if len(v6diffset) != 0:
        print("The ports accessible only via IPv6 are: ",v6diffset)
